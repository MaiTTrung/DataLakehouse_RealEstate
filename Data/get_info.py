...
def input_file():
    return os.path.join(directory, 'urls4.csv')
...
def save_to_csv(data, file_path, mode='a', header=True, encoding='utf-8-sig'):
    df = pd.DataFrame(data)
    df.to_csv(file_path, mode=mode, index=False, header=header, encoding= encoding)

def main():
    start_time = time.time()
    urls = extract_urls(1, 1000)  # update the range of URLs to extract
